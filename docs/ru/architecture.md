...
Политика кеша на каждый эндпоинт (Enabled, TtlSeconds, VaryByParameters, VaryByApiKey,
CoalesceRequests) задаётся в админке. Ключ кеша - это роут эндпоинта плюс нормализованные значения
...
Отрендеренные байты JSON кешируются через IResponseCache (сейчас в памяти, с границей
`ResponseCacheMaxBytes` - при заполнении вытесняются самые давно использованные записи; абстракция
позволяет распределённый бэкенд позже). Кеш управляется сервером; клиент не может его отключить.
