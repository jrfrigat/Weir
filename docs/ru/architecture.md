...
биндинг параметров, опциональное кэширование, телеметрию и потоковую сериализацию - и больше ничего.
...
HTTP-запрос
  ...
  2. AuthN/AuthZ     API-ключ -> поиск по хешу (кэш) -> проверка требуемых скоупов
  3. Биндинг         запрос (body/query/route/header/claim/const) -> WeirParameter[]  (вкл. TVP)
  4. Проверка кэша   если кэш эндпоинта включён -> ключ = роут + vary-by параметры (+ api-ключ)
...
Есть три провайдера: SQLite (по умолчанию, один узел), PostgreSQL и SQL Server (каждый - общая серверная
база для развёртываний с высокой доступностью, где несколько инстансов работают против одной
control-базы). У каждого свой раннер идемпотентных транзакционных миграций; провайдер выбирается через
...
Weir.Abstractions     серверные порты: IDbConnector, IControlPlaneStore, IWeirCallObserver,
  ...
  зависят от Abstractions:
    Weir.Core                    движок (резолв, биндинг, JSON-writer, оркестрация кэша)
    ...
    Weir.ControlPlane.SqlServer  реализация IControlPlaneStore + миграции (общее хранилище для HA)
...
AddWeirControlPlaneSqlite(), AddWeirControlPlanePostgres(), AddWeirControlPlaneSqlServer()). Оба
коннектора повторяют только транзиентные ошибки открытия подключения, поэтому процедура никогда не
...
Политика кэша на каждый эндпоинт (Enabled, TtlSeconds, VaryByParameters, VaryByApiKey,
CoalesceRequests) задаётся в админке. Ключ кэша - это роут эндпоинта плюс нормализованные значения
vary-by параметров (и, опционально, API-ключ); при включённом CoalesceRequests (по умолчанию) вызовы,
пришедшие пока ответ для того же ключа уже готовится, ждут его байты, а не выполняют объект каждый сам.
Отрендеренные байты JSON кэшируются через IResponseCache (сейчас в памяти, с границей
`ResponseCacheMaxBytes` - при заполнении вытесняются самые давно использованные записи; абстракция
позволяет распределённый бэкенд позже). Кэш управляется сервером; клиент не может его отключить.
