...
- Кэш ответов - на инстанс (in-memory), поэтому промах кэша на одном инстансе независим от других: эта
  часть стоит только hit ratio. Важна другая - инвалидация. Правка сразу очищает кэш того инстанса,
  который её обслужил, а остальные очищают свой на ближайшей перезагрузке каталога, поэтому
  устаревание ограничено `ReloadSeconds`, а не TTL. Исключение - принудительная очистка (действие
  **Очистить кэш** в админке и `POST /admin/api/cache/purge`): она не несёт изменения метаданных,
  которое заметила бы перезагрузка, поэтому опустошает только тот инстанс, который обработал вызов.
  Чтобы очистить весь парк, отправьте вызов на каждый инстанс.
