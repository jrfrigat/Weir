...
маппинг параметров, кэширование, телеметрия и сериализация.
...
Устанавливаемое PWA, которое раздаёт сам шлюз. Дашборд кормится по SignalR - метрики приходят по мере
изменения, а не по таймеру, - а остальное управляет эндпоинтами, ключами, скоупами, админами, логом
запросов, аудитом и настройками рантайма. Английский и русский, язык выбирается для каждого браузера
(скриншоты сняты на английском).

<p align="center">
  <img src="assets/screenshots/dashboard.png" width="900"
       alt="Дашборд Weir: здоровье подключений, метрики сервиса, живые графики пропускной способности и задержки, метрики по эндпоинтам">
  <br><em>Дашборд - пропускная способность, задержка и метрики по эндпоинтам, приходящие по мере изменения</em>
</p>

<p align="center">
  <img src="assets/screenshots/endpoints.png" width="900"
       alt="Список эндпоинтов: маршрут, подключение, объект БД, тип, число параметров и транспорты">
  <br><em>Эндпоинты - HTTP-маршруты, отображённые на процедуры, функции, таблицы и представления</em>
</p>

<p align="center">
  <img src="assets/screenshots/endpoint-editor.png" width="900"
       alt="Редактор эндпоинта: метод, маршрут, подключение, операция, объект, транспорты, отдача и кэширование">
  <br><em>Редактор на эндпоинте, доступном по всем трём транспортам</em>
</p>

<p align="center">
  <img src="assets/screenshots/settings.png" width="900"
       alt="Настройки рантайма: лимиты data plane, отдача ответа, кэш ответов, ограничение частоты и устойчивость подключений">
  <br><em>Настройки рантайма - применяются без перезапуска</em>
</p>
...
src/
  ...
  Weir.Abstractions/         Серверные порты: IDbConnector, IControlPlaneStore, телеметрия, кэш
  Weir.Core/                 Движок: резолв эндпоинтов, биндинг, стриминг JSON, кэш
...
Кэширование настраивается на стороне сервера для каждого эндпоинта (TTL плюс набор входных
