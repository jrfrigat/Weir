...
4. Создайте API-ключ и вызывайте endpoint, например:
   ...
   - `GET /api/stream?batches=5&rowsPerBatch=500&delayMs=1000` (пачки строк с паузой между ними; см.
     [Проверка потоковой передачи](#проверка-потоковой-передачи))
...
Команда `stream` проверяет, что ответ действительно стримится по всей цепочке - база, Weir и всё, что
стоит между Weir и клиентом. Она вызывает `GET /api/stream` из демо-базы (процедура
`sales.StreamBatches` отдаёт несколько пачек строк, каждая - отдельный result set, с паузой между
ними) и записывает, когда приходит каждая часть тела. Потоковый ответ приходит всплесками, разнесёнными
на паузу; ответ, который кто-то задержал, приходит одним всплеском в конце.

```sh
# Напрямую к Weir, затем через прокси - и сравнить:
...
# То же со сжатием (засекаются сжатые байты, как они пришли по сети):
dotnet run -- stream --url https://weir.example.com --compress

# Любой другой endpoint вместо демо-процедуры:
dotnet run -- stream --route "reports/export?year=2026"
```

Команда печатает заголовки, от которых зависит стриминг (`Transfer-Encoding`, `Content-Encoding`,
`X-Accel-Buffering`, `Server`), таблицу всплесков и вердикт: **Streamed** (код 0), **Buffered** (1),
**Cut off** (1 - тело оборвалось посередине, обычно из-за таймаута чтения у прокси, который короче
паузы) или **Inconclusive** (2 - ответ был слишком коротким, чтобы судить). Если ответ буферизуется,
проверьте режим отдачи endpoint'а, кэш и логирование результата, затем прокси; см.
[Потоковая передача через прокси](../docs/ru/deployment.md#потоковая-передача-через-прокси).
