...
/// <summary>
/// <see cref="IControlPlaneStore"/> backed by SQL Server, for deployments that keep Weir's own
/// metadata in SQL Server (and, with a shared database, several Weir instances against one control
/// database). Opens a pooled connection per operation and self-migrates on <see cref="InitializeAsync"/>.
/// Complex fields (parameters, cache policy, scope lists) are stored as JSON text columns; timestamps
/// are stored as ISO 8601 text for portability.
/// </summary>
public sealed class SqlServerControlPlaneStore : IControlPlaneStore
{
    ...
    /// <summary>The Microsoft.Data.SqlClient connection string.</summary>
    ...
    public SqlServerControlPlaneStore(IOptions<SqlServerControlPlaneOptions> options, TimeProvider clock)
    {
    ...
    /// <summary>
    /// Fixed resource name for the application lock that serializes migrations across instances that
    /// share this control database (so a rolling deploy in HA cannot run migrations concurrently).
    /// </summary>
    private const string MigrationLockResource = "WeirControlPlaneMigration";

    /// <summary>Duplicate-key error number raised by SQL Server on a unique-constraint violation.</summary>
    private const int SqlErrorDuplicateKey = 2627;

    /// <summary>Unique-index-violation error number raised by SQL Server (distinct from a PK/unique constraint).</summary>
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await conn.ExecuteAsync(new CommandDefinition("""
                IF OBJECT_ID(N'weir_schema_version', N'U') IS NULL
                CREATE TABLE weir_schema_version (version int NOT NULL);
                IF COL_LENGTH(N'weir_schema_version', N'singleton') IS NULL
                ALTER TABLE weir_schema_version ADD singleton bit NOT NULL DEFAULT 1;
                DELETE FROM weir_schema_version
                    WHERE version NOT IN (SELECT TOP (1) version FROM weir_schema_version ORDER BY version DESC);
                IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = N'UX_weir_schema_version_singleton' AND object_id = OBJECT_ID(N'weir_schema_version'))
                CREATE UNIQUE INDEX UX_weir_schema_version_singleton ON weir_schema_version (singleton);
                """, cancellationToken: cancellationToken));

            await conn.ExecuteAsync(new CommandDefinition("""
                IF OBJECT_ID(N'SchemaMigrations', N'U') IS NULL
                CREATE TABLE SchemaMigrations (Version int PRIMARY KEY, Checksum nvarchar(200) NOT NULL, AppliedAt nvarchar(40) NOT NULL);
                """, cancellationToken: cancellationToken));

            var version = await conn.ExecuteScalarAsync<int?>(
                new CommandDefinition("SELECT TOP (1) version FROM weir_schema_version", cancellationToken: cancellationToken));
            if (version is null)
            {
                await conn.ExecuteAsync(new CommandDefinition(
                    "IF NOT EXISTS (SELECT 1 FROM weir_schema_version) INSERT INTO weir_schema_version (singleton, version) VALUES (1, 0);",
                    cancellationToken: cancellationToken));
                version = 0;
            }
        ...
        {
            await conn.ExecuteAsync(new CommandDefinition(
                "EXEC sp_releaseapplock @Resource = 'WeirControlPlaneMigration', @LockOwner = 'Session';",
                cancellationToken: cancellationToken));
        }
    }
    ...
    private async Task VerifyOrBackfillChecksumsAsync(SqlConnection conn, int appliedVersion, CancellationToken cancellationToken)
    {
        ...
        var upTo = Math.Min(appliedVersion, SqlServerSchema.Migrations.Length);
        for (var applied = 1; applied <= upTo; applied++)
        {
            var checksum = Checksum(SqlServerSchema.Migrations[applied - 1]);
            ...
            {
                await conn.ExecuteAsync(new CommandDefinition(
                    "IF NOT EXISTS (SELECT 1 FROM SchemaMigrations WHERE Version = @Version) INSERT INTO SchemaMigrations (Version, Checksum, AppliedAt) VALUES (@Version, @Checksum, @AppliedAt)",
    ...
    /// <summary>Whether a SQL Server exception is a unique-constraint or unique-index violation.</summary>
    /// <param name="ex">The provider exception.</param>
    /// <returns>True if any contained error is a duplicate-key or unique-index error.</returns>
    private static bool IsUniqueViolation(SqlException ex)
    {
        foreach (SqlError error in ex.Errors)
        {
            if (error.Number is SqlErrorDuplicateKey or SqlErrorUniqueIndex)
            {
                return true;
            }
        }
    ...
    public async Task<IReadOnlyList<RequestLogEntry>> QueryRequestLogAsync(RequestLogQuery query, CancellationToken cancellationToken = default)
    {
        ...
        sql.Append(" ORDER BY Id DESC OFFSET 0 ROWS FETCH NEXT @Limit ROWS ONLY");
    ...
    public async Task<RefreshTokenRecord?> FindRefreshTokenAsync(string hash, CancellationToken cancellationToken = default)
    {
        ...
        return new RefreshTokenRecord
        {
            Id = Guid.Parse(row.Id),
            AdminId = Guid.Parse(row.AdminId),
            ExpiresAt = ParseDto(row.ExpiresAt),
            Revoked = row.RevokedAt is not null,
        };
    }
    ...
    {
        await using var conn = await OpenAsync(cancellationToken);
        // Single-row upsert: UPDATE the fixed Id = 1 row, INSERT it when absent.
        await conn.ExecuteAsync(new CommandDefinition("""
            UPDATE Settings SET Json = @Json, UpdatedAt = @UpdatedAt WHERE Id = 1;
            IF @@ROWCOUNT = 0
    ...
    /// <summary>Row shape for a refresh-token lookup by hash.</summary>
    private sealed class RefreshTokenRow
    {
        ...
        /// <summary>Owning admin id (GUID text).</summary>
        public string AdminId { get; set; } = "";
        /// <summary>Expiry timestamp (ISO-8601 text).</summary>
        public string ExpiresAt { get; set; } = "";
        /// <summary>Optional revocation timestamp (ISO-8601 text); non-null means revoked.</summary>
